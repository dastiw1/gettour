...
function () {
  ...
  _createClass(ConditionEventsListeners, [{
    ...
    value: function start() {
      ...
      var matches = 0;
      Object.keys(this.autoConditions).forEach(function (uuid) {
        var cond = _this3.autoConditions[uuid];

        if (cond.page_url == null || _this3.active !== uuid && _this3.testForPathname(cond)) {
          if (_this3.matchDate(cond)) {
    ...
    value: function clickListener(jsEvent) {
      ...
      Object.keys(this.manualConditions).forEach(function (uuid) {
        var cond = _this4.manualConditions[uuid];

        if (_this4.testForPathname(cond) && jsEvent.target.matches(cond.click_on_element.value)) {
          if (_this4.matchDate(cond)) {
            window.getTourEventBus.dispatchEvent('ConditionMatched', {
              uuid: uuid
            });
          }
...
var onboarding = {
  ...
  loadCondition: function loadCondition(event) {
    ...
    if (this.active.condition) {
      this.renderWidget(this.autoShowConditions[this.active.condition].link, this.options.launchAsExpanded);
      ...
      if (this.options.launchAsExpanded) {
        this.expandBlock();
      }
    }
  },
  ...
  renderWidget: function renderWidget(widgetUrl, asExpanded) {
    ...
    var widgetHtml = widgetTemplateLoader({
      widgetUrl: widgetUrl,
  ...
  initSystemEventListeners: function initSystemEventListeners() {
    ...
    $closeBtn.addEventListener('click', function () {
      if (_this4.block.classList.contains(_this4.expandClass)) {
        _this4.hideBlock();
      }
    });
    $launcher.addEventListener('click', function () {
      if (_this4.triggeredCount === 0 && !_this4.block.classList.contains(_this4.expandClass)) {
        _this4.expandBlock();
      }
    });
  },
  ...
  loadWidgetData: function loadWidgetData() {
    if (!this.hash) {
      (0, _utils.showError)('[Ошибка] hash отсутствует');
      return false;
    }
