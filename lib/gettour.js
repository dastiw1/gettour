...
function () {
  ...
  _createClass(ConditionEventsListeners, [{
    ...
    value: function filterByPath(conds) {
      ...
      return Object.keys(conds).filter(function (uuid) {
        var cond = conds[uuid];
        var path = window.location.pathname;

        var prevPath = _jsCookie.default.get(pathKey);

        return cond.page_url == null || prevPath !== path && _this3.testForPathname(cond, path);
      });
    }
  }, {
    key: "start",
    value: function start() {
      ...
      var matches = 0;
      this.filterByPath(this.autoConditions).forEach(function (uuid) {
        ...
        if (_this4.matchDate(cond)) {
          ...
          _jsCookie.default.set(pathKey, window.location.pathname);
...
var onboarding = {
  ...
  renderWidget: function renderWidget(widgetUrl, asExpanded) {
    ...
    var bots = this.ConditionEventsListeners.filterByPath(this.autoShowConditions).map(function (uuid) {
      var _this4$autoShowCondit = _this4.autoShowConditions[uuid],
          link = _this4$autoShowCondit.link,
          name = _this4$autoShowCondit.name;
      return {
        link: link,
        name: name
      };
  ...
  initSystemEventListeners: function initSystemEventListeners() {
    ...
    $menuBtn.addEventListener('click', function () {
      var activeClass = 'getchat-widget__bots--active';
      var $botsBlock = document.querySelector('.getchat-widget__bots');
      var method;
