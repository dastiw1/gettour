    ...
    this.loadWidgetData().then(function (data) {
      ...
      if (!_this.listenForMessages) {
        _this.listenForMessages = function (event) {
          ...
          this.__listenForHighlightRequests.call(this, event); // Слушать события кнопок чата


          this.__listenForActionClickedRequests.call(this, event); // Слушать события для Observer-а


          this.__listenForObserveRequests.call(this, event); // Слушать события для EVALUATE для выполнения userScript


          this.__listenForUserScriptEvaluateRequests.call(this, event); // Слушать события наличия новых сообщении


          this.__listenForNewMessages.call(this, event); // Слушать события подгрузки нового бота
