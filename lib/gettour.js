...
var onboarding = Object.assign({
  ...
  init: function init(hash) {
    ...
    this.loadWidgetData().then(function (data) {
      ...
      _this.active.status = data.widget_active; // this.options = Object.assign(this.options, data.widget_options);
      ...
      Object.keys(data.conditions).forEach(function (key) {
        data.conditions[key] = Object.assign({
          options: {
            alignment: _this.options.alignment
          }
        }, data.conditions[key]);
      });
      _this.autoShowConditions = data.conditions;

      if (!_this.active.status && Object.keys(_this.autoShowConditions).includes(_this.active.condition)) {
        return;
      }
      ...
      if (!_this.listenForMessages) {
        _this.listenForMessages = function (event) {
        ...
        window.addEventListener('message', _this.listenForMessages);
      } // Подписка на событие "когда условия виджета выполнены"


      window.getTourEventBus.addEventListener('ConditionMatched', _this.conditionMatchHandler); // Если не подходит под условия
  ...
  conditionMatchHandler: function conditionMatchHandler(e) {
    ...
    if (e != null && self.autoShowConditions[e.detail.uuid].onClick && self.active.condition === e.detail.uuid && self.active.condition) {
  ...
  renderWidget: function renderWidget(chatUrl, asExpanded) {
    ...
    frame.onload = function () {
      if (!_this4.block.classList.contains(_this4.expandClass) && asExpanded) {
        _this4.expandBlock();
      }
    }; // запуск слушателей
  ...
  showAvailableBots: function showAvailableBots() {
    ...
    var answers = this.ConditionEventsListeners.filterByPath(this.autoShowConditions, true).map(function (uuid) {
      ...
      var _this9$autoShowCondit = _this9.autoShowConditions[uuid],
          bot_id = _this9$autoShowCondit.bot_id,
          name = _this9$autoShowCondit.name,
          start = _this9$autoShowCondit.start;
      return {
        answer: {
          answer_id: answer_id,
          bot_id: bot_id,
